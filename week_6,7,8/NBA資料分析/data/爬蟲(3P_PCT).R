...
last <- "&SeasonType=Regular+Season&StatCategory=FG3_PCT"
#3P_PCT
for(id in c(from : to)){
  if(((id+1)%%1000)%%100<10){
  ...
  file_path <- paste0("C:/Users/user/Desktop/week_6,7,8/data/3P_PCT/",id,"-",id1,".csv")
  my.df <- fromJSON(url)
  if(nrow(my.df$resultSet$rowSet)<50){
    
  }else{
    data <- as.data.frame(my.df$resultSet$rowSet[,2:27])
